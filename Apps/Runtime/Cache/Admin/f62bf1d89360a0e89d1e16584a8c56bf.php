                    ...
                    <div class="col-sm-12">
                        ...
                        <table class="table table-bordered table-striped">
                            ...
                            <?php if(is_array($data)): foreach($data as $key=>$row): ?><tr>
                                    ...
                                    <td>
                                        <?php $payments = json_decode($row['type'],true); $length = count($payments); $configs = C('PAYMENT_TYPE'); foreach($payments as $k => $v){ if($payments[$k] == 1){ echo '<div class="pay_icon pay_icon_'.$k.'" title="'.$configs[$k]['name'].'"></div>'; } } ?>
                                    </td>
                                    <td>

                                        <a class="btn btn-sm btn-success" title="编辑" href="/App/payment_edit_form/id/<?php echo ($row["id"]); ?>">
                                            <i class="fa-edit"></i>
                                        </a>

                                        <button class="btn btn-sm btn-red" title="删除" onclick="MSGBOX.show('是否真的要删除？',function(){
                                location.href='/App/payment_del/id/<?php echo ($row["id"]); ?>'
                            });">
                                            <i class="fa-remove"></i>
                                        </button>
                                    </td>
