        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                ...
                <div class="row">
                    ...
                    <div class="col-sm-12">
                        <form method="get" action="/Member/orderlist" autocomplete="false">
                        订单号： <input type="text" class="seachInput" name="order_no"  value="<?php echo ($_GET['order_no']); ?>" />&nbsp;&nbsp;
                            UID： <input type="text" class="seachInput" name="uid"  value="<?php echo ($_GET['uid']); ?>" />&nbsp;&nbsp;

                            <?php $trade_status = $_GET['trade_status']; ?>
                            交易状态： <select name="trade_status" class="seachInput">
                            <option value="999">全部</option>
                            <option value="1" <?php echo ($trade_status==1?'selected="selected"':''); ?>>已完成</option>
                            <option value="-1" <?php echo ($trade_status==-1?'selected="selected"':''); ?>>未完成</option>
                        </select>&nbsp;&nbsp;
                            <button class="btn btn-success " type="submit">Search</button></form><br /><br />
                        ...
                        <table class="table table-bordered table-striped">
                            ...
                            <tr>
                                ...
                                <th>用户(UID)</th>
                                <th>商品名称</th>
                                <th class='title_2'>充值</th>
                                <th class='title_2'>消费</th>
                                <th>方式</th>
                                <th class='title_2'>状态</th>
                                <th>时间</th>
                                <th>操作</th>
                            </tr>
                            </thead>
                            ...
                            <?php if(is_array($data)): foreach($data as $key=>$row): ?><tr>
                                    ...
                                    <td><?php echo ($row["username"]); ?><br />(<?php echo ($row["uid"]); ?>)</td>
                                    <td><?php echo ($row["subject"]); ?></td>
                                    <td><?php echo ($row["total_fee"]); ?></td>
                                    <td><?php echo ($row["game_total_fee"]); ?></td>
                                    <td><?php echo $paytype[$row['payment_type']] ?></td>
                                    <td><?php echo ($row["trade_status"]); ?></td>

                                    <td><?php echo (date("Y/m/d H:i",$row["time"])); ?></td>
                                    <td><a class="btn btn-blue btn-icon btn-icon-standalone btn-icon-standalone-right btn-xs" href="/Member/orderview/id/<?php echo ($row["id"]); ?>">
                                        <i class="fa-search"></i>
                                        <span>详情</span>
                                    </a></td>
